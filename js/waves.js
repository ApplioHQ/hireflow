...
// is rewritten as a self-contained DOM/SVG animation. Enhancements over the original:
// gradient strokes, smooth (curved) lines, a glowing cursor dot, edge-fade mask
// (via CSS), and an off-screen pause for performance. Auto-inits on #hero-waves.
(function () {
  ...
  function initWaves(container, opts) {
    ...
    // Gradient stroke: indigo -> violet -> pink across the field for subtle depth.
    const defs = document.createElementNS(NS, 'defs');
    const grad = document.createElementNS(NS, 'linearGradient');
    grad.id = 'wave-grad'; grad.setAttribute('gradientUnits', 'userSpaceOnUse');
    [['0%', '#6366f1'], ['50%', '#8b5cf6'], ['100%', '#c084fc']].forEach(function (s) {
    ...
    function setSize() {
      ...
      grad.setAttribute('x2', String(bounding.width)); grad.setAttribute('y2', String(bounding.height));
    }
    function setLines() {
      lines = [];
      ...
      for (let i = 0; i < totalLines; i++) {
        ...
        // Slight per-line opacity variation adds depth.
        path.setAttribute('stroke-opacity', (0.10 + (i % 5) * 0.018).toFixed(3));
    ...
    function movePoints(time) {
      lines.forEach(points => {
        points.forEach(p => {
          ...
          p.cursor.x = Math.min(60, Math.max(-60, p.cursor.x)); p.cursor.y = Math.min(60, Math.max(-60, p.cursor.y));
        });
      });
    }
    ...
    function drawLines() {
      lines.forEach((points, idx) => {
        ...
        // Smooth quadratic curve through point midpoints (silkier than straight segments).
        let prev = moved(points[0], false);
        let d = `M ${prev.x.toFixed(2)} ${prev.y.toFixed(2)}`;
        for (let i = 1; i < points.length; i++) {
          const c = moved(points[i], true);
          const mx = (prev.x + c.x) / 2, my = (prev.y + c.y) / 2;
          d += ` Q ${prev.x.toFixed(2)} ${prev.y.toFixed(2)} ${mx.toFixed(2)} ${my.toFixed(2)}`;
          prev = c;
        }
        d += ` L ${prev.x.toFixed(2)} ${prev.y.toFixed(2)}`;
    ...
    function tick(time) {
      ...
      if (mouse.set) { dot.style.transform = `translate(${mouse.sx}px, ${mouse.sy - window.scrollY}px)`; dot.style.opacity = '1'; }
    ...
    setSize(); setLines();
    if (reduce) { movePoints(0); drawLines(); return; } // static, no loop
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (el) initWaves(el, { xGap: 30, yGap: 30 });
  });
})();
